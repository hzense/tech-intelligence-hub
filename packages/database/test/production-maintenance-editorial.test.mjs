...
const pending = ['0025_editorial_signal_publication.sql'];
const identity = { database: 'hzense', user: 'migrator' };
const policy = { host: 'fixture.invalid', port: '5432', ...identity };
const backup = 'synthetic-editorial-publication-backup';
const binding = editorialTargetBinding(policy, identity, backup);
const plan = editorialMigrationPlan(pending, migrations, binding);
...
const approval = {
  ...
  riskAcceptance: {
    scope: 'editorial-publication-production-launch',
    accepted: true,
...
describe('independent 0025 maintenance authority', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    calls.policy = policy;
  ...
  it('pins twenty-five files and accepts only reviewed 0024 or 0025', () => {
    expect(migrations).toHaveLength(26);
    expect(() => materialReviewMigrationPlan(pending, migrations, binding)).toThrow(
      'manifest-required',
    );
    const both = ['0024_material_review_proposals.sql', ...pending];
    expect(() => editorialMigrationPlan(both, migrations, binding)).toThrow('scope-required');
    ...
    expect(
      requireEditorialMigrationScope({ pendingMigrations: pending }, migrations, plan, binding),
    ).toEqual(plan);
    for (const names of [
      [],
      Array(1),
      ...
      [...pending, '0026_future.sql'],
    ])
      expect(() => editorialMigrationPlan(names, migrations, binding)).toThrow(
  ...
  it('binds every plan field and refuses altered or extended migration artifacts', () => {
    ...
    for (const artifact of [
      ...
      [...migrations, { name: '0026_future.sql', checksum: 'd'.repeat(64), sql: '' }],
      ...
      expect(() => editorialMigrationPlan(pending, artifact, binding)).toThrow('manifest-required');
    ...
    expect(
      editorialMigrationPlan(
        pending,
        migrations,
        editorialTargetBinding(policy, identity, `${backup}-new`),
      ).planFingerprint,
  ...
  it('checks the independent target binding before ACL capture', async () => {
    ...
    ).rejects.toThrow('editorial-publication-target-mismatch');
    expect(calls.capture).toHaveBeenCalledOnce();
  });

  it('requires the editorial-publication risk scope and binds the reviewed target', () => {
  ...
  it('rechecks the locked execution artifact before applying DDL', async () => {
    ...
    calls.inspect.mockResolvedValue({ ...identity, pendingMigrations: pending });
    calls.load.mockResolvedValue(migrations);
    calls.verify.mockResolvedValue({ migrationCount: 26 });
    const ddl = vi.fn();
