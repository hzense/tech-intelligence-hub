...
const migrations = Object.entries(JSON.parse(readFileSync(new URL('checksums.json', root), 'utf8')))
  .filter(([name]) => name < '0023_')
  .map(([name, checksum]) => ({ name, checksum, sql: readFileSync(new URL(name, root), 'utf8') }));
const pending = ['0022_candidate_enrichment_runs.sql'];
const identity = { database: 'hzense', user: 'migrator' };
const policy = { host: 'fixture.invalid', port: '5432', ...identity };
const backup = 'synthetic-candidate-enrichment-backup';
const binding = candidateEnrichmentTargetBinding(policy, identity, backup);
const plan = candidateEnrichmentMigrationPlan(pending, migrations, binding);
...
const approval = {
  ...
  recoveryPolicy: 'accept-unverified-candidate-enrichment',
  riskAcceptance: {
...
describe('independent 0022 maintenance authority', () => {
  it('pins twenty-three files and accepts only the single 0022 migration', () => {
    ...
    for (const names of [
      ...
      expect(() => candidateEnrichmentMigrationPlan(names, migrations, binding)).toThrow(
    ...
    expect(() =>
      candidateEnrichmentMigrationPlan(
        ...
        migrations.map((migration, index) =>
          index === 22 ? { ...migration, sql: `${migration.sql} ` } : migration,
        ),
        binding,
      ),
  ...
  it('requires the candidate-enrichment risk scope and binds the reviewed target', () => {
  ...
  it('rechecks the locked execution artifact before applying DDL', async () => {
    ...
    calls.verify.mockResolvedValue({ migrationCount: 23 });
    const ddl = vi.fn();
