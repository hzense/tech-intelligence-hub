...
// This reviewed 0000–0018 fixture must not gain authority when 0019 is introduced.
const migrations = Object.freeze(
  Object.entries(currentManifest)
    .filter(([name]) => name < '0019_')
    .map(([name, checksum]) =>
...
const binding = taskManagementTargetBinding(policy, identity, backupId);
const plan = taskManagementMigrationPlan(pending, migrations, binding);
const riskAcceptance = Object.freeze({
  scope: 'task-management-production-launch',
  accepted: true,
...
function environment(operation = 'migrate', changes = {}) {
  return {
    ...
    MAINTENANCE_APPROVAL: JSON.stringify({
      ...
      recoveryPolicy: 'accept-unverified-task-management',
      riskAcceptance,
      ...binding,
...
beforeEach(() => {
  ...
  calls.verify.mockResolvedValue({ migrationCount: 19, tableCount: 48 });
...
describe('independent 0016–0018 migration artifact and target binding', () => {
  it('pins all nineteen SQL files and reproduces the independent public fingerprints', () => {
    expect(migrations).toHaveLength(19);
    expect(migrations.at(-1)).toMatchObject({
      name: '0018_generation_task_visibility.sql',
      checksum: '5a945ab6271cfbab0f58e3e11e78d7ff3eeb61041eec18f449b4ef44a4bc15e9',
    });
    const manifestFingerprint = hash(
      `hzense/task-management-migration-manifest/v1\0${JSON.stringify(
    ...
    expect(plan).toEqual({
      ...
      planFingerprint: hash(
        `hzense/task-management-migration-plan/v1\0${JSON.stringify({
          manifestFingerprint,
    ...
    expect(
      taskManagementTargetBinding({ ...policy, host: 'GENERATION.INVALID' }, identity, backupId),
    ).toEqual(binding);
  });
  ...
  it.each([
    ...
    ['future', [...pending, '0019_future.sql']],
    ['duplicate', [...pending, ...pending]],
    ['not an array', pending[0]],
    ['missing', undefined],
  ])('rejects the %s pending plan', (_, actualPending) => {
    expect(() => taskManagementMigrationPlan(actualPending, migrations, binding)).toThrow(
  ...
  it.each([
    ...
    [
      ...
      migrations.map((entry, index) =>
        index === 18 ? { ...entry, name: '0018_other.sql' } : entry,
      ),
    ],
    [
      'claimed checksum',
      migrations.map((entry, index) =>
        index === 18 ? { ...entry, checksum: 'f'.repeat(64) } : entry,
      ),
    ],
    [
      'changed generation SQL',
      migrations.map((entry, index) =>
        index === 18 ? { ...entry, sql: `${entry.sql}\n` } : entry,
      ),
    ],
    [
    ...
    [
      'missing SQL bytes',
      migrations.map((entry, index) => (index === 18 ? { ...entry, sql: undefined } : entry)),
    ],
  ])('rejects %s even with the approved pending name', (_, artifact) => {
    expect(() => taskManagementMigrationPlan(pending, artifact, binding)).toThrow(
  ...
  it.each(['manifestFingerprint', 'planFingerprint', 'targetFingerprint', 'backupIdSha256'])(
    ...
    (key) => {
      ...
      ).toThrow('task-management-migration-plan-mismatch');
    },
  );

  it.each(['host', 'port', 'database', 'user'])('requires target component %s', (key) => {
    expect(() => taskManagementTargetBinding({ ...policy, [key]: '' }, identity, backupId)).toThrow(
  ...
  it.each(['database', 'user'])(
    ...
    (key) => {
      expect(() =>
        taskManagementTargetBinding(policy, { ...identity, [key]: 'other' }, backupId),
      ).toThrow('task-management-target-required');
    },
  );
  it.each([
    undefined,
    ...
    expect(() => taskManagementTargetBinding(policy, identity, backup)).toThrow(
...
describe('generation recovery declarations remain explicit and run bound', () => {
  ...
  it('does not authorize search writes', () => {
    expect(() => validateMaintenanceRequest(environment('search-apply'), now)).toThrow(
      'task-management-operation-required',
    );
  });
  ...
  it.each(['manifestFingerprint', 'planFingerprint'])(
    ...
    (key) => {
      ...
      ).toThrow('reviewed-task-management-plan-required');
    },
  );
...
describe('real hosted generation executor with synthetic database adapters', () => {
  it('issues the exact read-only plan without migration, schema verification, or ACL capture', async () => {
    ...
    ).toEqual({ operation: 'preflight', status: 'succeeded', pendingMigrationCount: 3, ...plan });
  ...
  it.each([undefined, '', 'placeholder-backup'])(
    ...
    async (backup) => {
      ...
      ).toEqual({ operation: 'preflight', status: 'succeeded', pendingMigrationCount: 3 });
  ...
  const generationBinding = signalGenerationTargetBinding(policy, identity, backupId);
  ...
  it.each([
    [
      'generation',
      {
        ...
        riskAcceptance: { ...riskAcceptance, scope: 'signal-generation-production-launch' },
        ...signalGenerationMigrationPlan(
          ['0015_signal_generation.sql'],
          migrations.slice(0, 16),
          generationBinding,
        ),
      },
  ...
  ])('refuses the historical %s approval for 0016–0018 before DDL', async (_, changes, gate) => {
  ...
  it.each(['host', 'port', 'database', 'user'])(
    ...
    async (key) => {
      ...
      await expect(execute()).rejects.toThrow('task-management-migration-plan-mismatch');
      expect(calls.ddl).not.toHaveBeenCalled();
    },
  );
  it('refuses backup replacement even when the separate approval backup hash is updated', async () => {
    ...
    await expect(execute(env)).rejects.toThrow('task-management-migration-plan-mismatch');
    expect(calls.ddl).not.toHaveBeenCalled();
  });
  it.each(['database', 'user'])(
    ...
    async (key) => {
      ...
      await expect(execute()).rejects.toThrow('task-management-target-required');
      expect(calls.ddl).not.toHaveBeenCalled();
    },
  );
  it.each([[], [...pending, '0019_future.sql']].map((value) => [value]))(
    'refuses an advanced or expanded locked pending plan %j',
    async (actualPending) => {
      calls.lockedPending = actualPending;
      await expect(execute()).rejects.toThrow('task-management-migration-scope-required');
      expect(calls.ddl).not.toHaveBeenCalled();
    },
  );
  it.each([
    [
      ...
      migrations.map((entry, index) => (index === 18 ? { ...entry, sql: 'SELECT 1;' } : entry)),
    ],
    ...
    async (_, artifact) => {
      ...
      await expect(execute()).rejects.toThrow('task-management-migration-manifest-required');
      expect(calls.load).toHaveBeenCalledOnce();
      expect(calls.ddl).not.toHaveBeenCalled();
    },
  );
  ...
  it('publishes only a bounded gate when target binding fails', async () => {
    ...
    expect(failure).toEqual({
      status: 'blocked',
      gate: 'task-management-migration-plan-mismatch',
    });
  });
});
...
describe('task batch partial completion', () => {
  it.each([1, 2])('requires fresh approval for remaining suffix of length %s', (length) => {
    const remaining = pending.slice(-length);
    const resumed = taskManagementMigrationPlan(remaining, migrations, binding);
    expect(resumed.planFingerprint).not.toBe(plan.planFingerprint);
    expect(() =>
      requireTaskManagementMigrationScope(
        { pendingMigrations: remaining },
        migrations,
        plan,
        binding,
      ),
  ...
  it('rejects non-suffix subset', () => {
    expect(() => taskManagementMigrationPlan(pending.slice(0, 2), migrations, binding)).toThrow(
      'task-management-migration-scope-required',
    );
  });
  it('refuses a changed valid suffix after lock', async () => {
