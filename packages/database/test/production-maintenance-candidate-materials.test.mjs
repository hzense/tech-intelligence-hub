...
const migrations = Object.entries(JSON.parse(readFileSync(new URL('checksums.json', root), 'utf8')))
  .filter(([name]) => name < '0024_')
  .map(([name, checksum]) => ({ name, checksum, sql: readFileSync(new URL(name, root), 'utf8') }));
const pending = ['0023_candidate_materials.sql'];
const identity = { database: 'hzense', user: 'migrator' };
const policy = { host: 'fixture.invalid', port: '5432', ...identity };
const backup = 'synthetic-candidate-materials-backup';
const binding = candidateMaterialsTargetBinding(policy, identity, backup);
const plan = candidateMaterialsMigrationPlan(pending, migrations, binding);
...
const approval = {
  ...
  recoveryPolicy: 'accept-unverified-candidate-materials',
  riskAcceptance: {
...
describe('independent 0023 maintenance authority', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    calls.policy = policy;
  ...
  it('pins twenty-four files and accepts only the single 0023 migration', () => {
    ...
    for (const names of [
      ...
      [...pending, '0024_future.sql'],
    ])
      expect(() => candidateMaterialsMigrationPlan(names, migrations, binding)).toThrow(
    ...
    expect(() =>
      candidateMaterialsMigrationPlan(
        ...
        migrations.map((migration, index) =>
          index === 23 ? { ...migration, sql: `${migration.sql} ` } : migration,
        ),
        binding,
      ),
  ...
  it('binds every plan field and refuses altered or extended migration artifacts', () => {
    ...
    for (const artifact of [
      ...
      [...migrations, { name: '0024_future.sql', checksum: 'd'.repeat(64), sql: '' }],
      ...
      expect(() => candidateMaterialsMigrationPlan(pending, artifact, binding)).toThrow(
        'manifest-required',
      );
    ...
    expect(
      candidateMaterialsMigrationPlan(
        pending,
        migrations,
        candidateMaterialsTargetBinding(policy, identity, `${backup}-new`),
      ).planFingerprint,
  ...
  it('checks the independent target binding before ACL capture', async () => {
    ...
    ).rejects.toThrow('candidate-materials-target-mismatch');
    expect(calls.capture).toHaveBeenCalledOnce();
  });

  it('requires the candidate-materials risk scope and binds the reviewed target', () => {
    expect(validateMaintenanceRequest(env, now).operation).toBe('migrate');
    for (const change of [
      {
        riskAcceptance: {
          ...approval.riskAcceptance,
          scope: 'candidate-enrichment-production-launch',
        },
      },
  ...
  it('rechecks the locked execution artifact before applying DDL', async () => {
    ...
    calls.inspect.mockResolvedValue({ ...identity, pendingMigrations: pending });
    calls.load.mockResolvedValue(migrations);
    calls.verify.mockResolvedValue({ migrationCount: 24 });
    const ddl = vi.fn();
