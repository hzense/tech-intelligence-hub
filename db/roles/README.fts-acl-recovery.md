...
历史开发状态（绑定下述提交）：**开发候选；代码静态自审与 PostgreSQL CI 已通过，当时独立审核和 Neon 隔离演练尚未完成。**
...
上述记录不是实时审批或演练状态检查。不得根据本文件设置 `aclRecoveryReviewed` / `restoreRehearsed`，也不得将候选 SQL 接入生产自动执行。
没有新建本地维护 CLI，也不需要 `.env`。实际执行仍只能走获批的线上维护流程。

2026-09-10 已开发独立的[线上只读验证入口](../../docs/RECOVERY_VERIFICATION.md)，
用于 R0–R3 双采集和运行时角色恢复后的状态验证；它不执行本恢复 SQL，未声明合并或线上实跑。
...
- [restore_fts_reader_acl.sql](./restore_fts_reader_acl.sql) 只有一个权限修改语句：
  从 `hzense_runtime` 撤销 `public.search_documents` 的十二列 `SELECT` 权限，显式使用 `RESTRICT`。
- 保留专题表的五列权限、其他角色、平台提供方权限及默认 ACL。不删除 FTS 数据库结构、索引或业务行，
  不修改迁移记录，不恢复早期较宽的 PUBLIC 权限，不是通用备份恢复器。
- 仅当 R1 → R2 完整审查证明前向配置只增加这些列权限时适用。若权限规范化
  同时改变其他 ACL，必须另行开发并审核；后置指纹不同会使整个恢复事务失败。
- 要求 PostgreSQL 18、固定数据库 `hzense`、直接以数据库与表所有者
  `hzense_migrator` 认证、正常的运行时角色、FTS 18 列结构与迁移记录。
- 拒绝搜索表的表级有效权限、运行时角色的间接成员关系、转授权选项（`grant option`）、额外列访问、
  投影表继承或行级安全策略（RLS）、已启用的事件触发器、错误目标、过期声明与目录漂移。
  唯一允许的平台提供方成员关系与前向配置一致：`neondb_owner` 对运行时角色的
  仅管理关系（`admin-only`），由 `cloud_admin` 授予，不允许继承或切换角色（`inherit` / `set`）。

## 同一数据库模式的实际证据

在**同一个隔离分支、同一条时间线**，用独立只读连接采集：

1. R1：FTS 迁移后、前向运行时角色 ACL 配置前。
2. R2：前向运行时角色 ACL 配置后。人工审核原始系统目录差异，确认只增加十二列权限。
3. 经批准执行恢复后另采 R3，必须与 R1 比对，并用运行时角色自身认证连接验证读取与拒绝矩阵。

[fts_acl_recovery_state.sql](./fts_acl_recovery_state.sql) 用只读事务输出目录指纹、
分类计数/指纹与 Neon 目标摘要，不输出原始配置值或业务行。它只用于本候选的漂移核对，
不能取代现有完整 ACL 双采集或实际数据库模式与数据校验。R1/R2 审核必须同时保存现有采集器
产生的完整目录材料；只有摘要无法审查实际差异。

指纹格式 `hzense-fts-acl-state/v1` 与归档 `baseline.json` 的格式不同，不能混用。
范围为当前库非系统模式、关系、列、类型与例程的 ACL 和身份元数据，
加全局角色、成员关系、角色数据库设置、数据库 ACL、默认权限、策略与继承关系。
`NULL` 与显式 ACL 不合并；排序固定使用 `C` 排序规则；时区固定为 UTC。包含 OID 与服务端版本，
不能跨分支、重置、升级或数据库模式变更直接复用。系统模式权限、函数实现、完整 DDL、
业务数据、密码及云端拓扑不在这个摘要的验收范围。

## 受保护声明

以下是同一执行连接的会话设置，不是代码内常量，不应提交实际值。
线上执行入口尚未接入；不要在控制台填猜测值后直接运行候选 SQL。

| 会话设置（统一前缀 `hzense.acl_recovery.`） | 来源                                                     |
| ------------------------------------------- | -------------------------------------------------------- |
| `before_fingerprint`                        | 已审核 R2 的新格式目录指纹                               |
| `after_fingerprint`                         | 同一目标、同一数据库模式的真实 R1 新格式目录指纹         |
...
相同的事务级咨询锁（`advisory lock`），锁定搜索表和专题表，然后做前后有效权限与目录核对。
后置失败抛异常且不吞错，不能提交部分 `REVOKE`。发生错误后执行客户端必须停止并回滚或关闭
连接，不可“继续执行剩余语句”或手动提交。提交成功后仍需新的独立 R3 采集。

表锁会阻塞两张投影表的读写，所以只准在已审批的隔离目标测试。咨询锁依赖各方协作，
不能阻止平台提供方或不遵守协议的其他管理员修改全局角色、其他对象；运行窗口必须冻结
相关 DDL/ACL，执行后再独立复核。此脚本不声称可消除所有跨会话的检查与使用时差（TOCTOU）风险。
...
或存在其他差异，指纹仍可能不匹配；应停止审查，不能归一化掉差异以制造通过。

## 测试边界与后续门禁

下列门禁是未来如需使用候选 SQL 时的前提，不代表本次重新安排已取消的恢复验证或演练。

- 静态测试核对唯一 `REVOKE`、十二列允许列表、前后查询与只读采集完全一致、目标/超时/身份
  防护及无凭据、无批量恢复、无动态执行。通过不代表 SQL 已在 PostgreSQL 执行。
- `fts-acl-recovery.integration.test.mjs` 接入现有 `test:migrations`，只使用 CI 的一次性
  PostgreSQL 18，拒绝替换已有固定名称的测试夹具。未提供隔离环境时跳过。
- 原样脚本在普通 PostgreSQL 必须拒绝 Neon 身份缺失/伪造。事务核心测试仅在测试内移除
  Neon 专用身份片段，保留真实所有者、数据库、权限和事务防护，验证恢复、后置失败回滚等。
  这个明确的测试替换不进入发布 SQL；核心测试不是完整 Neon 成功路径验收。
- 仍需真实 Neon 身份能力核验、R1/R2 材料审查、外部独立评审、线上入口与新审批、
  隔离正反例演练、运行时角色自身登录和 R3 独立采集，之后才能讨论解除生产门禁。
