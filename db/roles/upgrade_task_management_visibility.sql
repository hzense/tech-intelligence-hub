-- One-time 0016–0018 role upgrade, after hosted migration and independent verify.
-- Authenticate as the hzense database owner. Preserve all existing grants;
-- refuse privilege drift and already-upgraded roles instead of normalizing ACLs.
-- User accepted unverified recovery only for this batch on 2026-09-19.
BEGIN;
SET LOCAL search_path = pg_catalog, pg_temp;
SET LOCAL statement_timeout = '30s';
DO $task_management_upgrade$
BEGIN
  ...
  IF NOT EXISTS(SELECT 1 FROM public.hzense_schema_migrations WHERE name='0016_generation_cancelled_recreation.sql' AND checksum='91fed92118f856d3eecdfd7c7ec5394f6e36a93958282fb70958cbdcda849c4b') THEN RAISE EXCEPTION 'Verify task management migration ledger'; END IF;
  IF NOT EXISTS(SELECT 1 FROM public.hzense_schema_migrations WHERE name='0017_import_task_visibility.sql' AND checksum='fef4f88c9592e03f79259787f31ab5974c187bdf6c8f9e9697c6b8faf5983012') THEN RAISE EXCEPTION 'Verify task management migration ledger'; END IF;
  IF NOT EXISTS(SELECT 1 FROM public.hzense_schema_migrations WHERE name='0018_generation_task_visibility.sql' AND checksum='5a945ab6271cfbab0f58e3e11e78d7ff3eeb61041eec18f449b4ef44a4bc15e9') THEN RAISE EXCEPTION 'Verify task management migration ledger'; END IF;
  IF EXISTS(SELECT 1 FROM (VALUES ('import_batches'),('signal_generation_runs')) t(name)
    WHERE NOT EXISTS(SELECT 1 FROM pg_attribute a JOIN pg_class c ON c.oid=a.attrelid JOIN pg_namespace n ON n.oid=c.relnamespace
      WHERE n.nspname='public' AND c.relname=t.name AND c.relkind='r'
        AND c.relowner=(SELECT oid FROM pg_roles WHERE rolname=current_user)
        AND a.attname='deleted_at' AND a.atttypid='timestamptz'::regtype AND NOT a.attnotnull AND NOT a.attisdropped)) THEN
    RAISE EXCEPTION 'Expected owner-controlled visibility columns required';
  END IF;
END;
$task_management_upgrade$;

-- Exact pre-upgrade role contract.
...
BEGIN
  ...
  IF NOT FOUND OR NOT target_role.rolcanlogin OR target_role.rolinherit OR target_role.rolconnlimit<>2
    ...
    RAISE EXCEPTION 'Import administrator role contract mismatch';
  END IF;
  IF (SELECT count(*) FROM pg_catalog.pg_auth_members WHERE member=target OR roleid=target)>1
    ...
    RAISE EXCEPTION 'Import administrator membership contract mismatch';
  END IF;
  IF NOT has_database_privilege(target,current_database(),'CONNECT')
    ...
    RAISE EXCEPTION 'Import administrator database privilege contract mismatch';
  END IF;
  ...
  IF EXISTS(SELECT 1 FROM pg_catalog.pg_database d WHERE d.datname<>current_database() AND d.datallowconn
    ...
    RAISE EXCEPTION 'Import administrator has unsafe privileges on another connectable database';
  END IF;
  IF NOT has_schema_privilege(target,'public','USAGE')
    ...
    RAISE EXCEPTION 'Import administrator schema privilege contract mismatch';
  END IF;
  -- Seven private tables only; originals, outputs and audit remain append-only.
  ...
  IF EXISTS(SELECT 1 FROM pg_proc p CROSS JOIN LATERAL aclexplode(p.proacl) a WHERE a.grantee=target)
    ...
    RAISE EXCEPTION 'Import administrator function privilege contract mismatch';
  END IF;
END;
$import_admin_verify$;
...
DECLARE
  ...
  checked_privilege text;
  expected boolean;
  allowed_columns jsonb := '{"signal_generation_runs":{"SELECT":["id","owner_id","batch_id","item_id","source_fence","source_hash","profile_id","profile_revision","generation_version","fingerprint","snapshot","configuration","status","lease_token","lease_until","budget_day","reserved_microusd","charged_microusd","result","error_code","created_at","finished_at"],"INSERT":["id","owner_id","batch_id","item_id","source_fence","source_hash","profile_id","profile_revision","generation_version","fingerprint","snapshot","configuration"],"UPDATE":["status","lease_token","lease_until","budget_day","reserved_microusd","charged_microusd","result","error_code","finished_at"]}}'::jsonb;
BEGIN
  -- Re-read role attributes and memberships after GRANT. No role mutation is
  -- allowed to bypass the commit gate; this does not prevent later drift.
  SELECT * INTO target_role FROM pg_roles WHERE oid=target;
  ...
  END IF;
  IF EXISTS(SELECT 1 FROM pg_class c CROSS JOIN LATERAL aclexplode(c.relacl) a WHERE a.grantee=target)
    OR (SELECT count(*) FROM pg_attribute c CROSS JOIN LATERAL aclexplode(c.attacl) a WHERE a.grantee=target)<>43
    OR EXISTS(SELECT 1 FROM pg_attribute col JOIN pg_class c ON c.oid=col.attrelid JOIN pg_namespace n ON n.oid=c.relnamespace
      CROSS JOIN LATERAL aclexplode(col.attacl) a WHERE a.grantee=target AND
      (n.nspname<>'public' OR c.relkind<>'r' OR col.attnum<=0 OR col.attisdropped
        OR a.grantor<>c.relowner OR a.is_grantable
...
GRANT SELECT(deleted_at),UPDATE(deleted_at) ON public.import_batches TO hzense_import_admin;
GRANT SELECT(deleted_at),UPDATE(deleted_at) ON public.signal_generation_runs TO hzense_generation_admin;

-- Exact post-upgrade role contract; any mismatch rolls back both grants.
DO $import_admin_verify$
DECLARE
  target oid := 'hzense_import_admin'::regrole;
  target_role pg_roles%ROWTYPE;
  ...
  checked_privilege text;
  expected boolean;
  allowed_columns jsonb := '{"import_batches":{"SELECT":["id","owner_id","fingerprint","intent","configuration","cancelled","created_at","deleted_at"],"INSERT":["id","owner_id","fingerprint","intent","configuration"],"UPDATE":["cancelled","deleted_at"]},"import_items":{"SELECT":["id","batch_id","position","kind","declaration","status","fence","created_at"],"INSERT":["id","batch_id","position","kind","declaration","status"],"UPDATE":["status","fence"]},"import_documents":{"SELECT":["item_id","object_key","object_version","sha256","byte_size","format","metadata","created_at"],"INSERT":["item_id","object_key","object_version","sha256","byte_size","format","metadata"]},"import_attempts":{"SELECT":["item_id","fence","parser_version","status","lease_until","budget_day","reserved_microusd","charged_microusd","error_code","created_at","finished_at"],"INSERT":["item_id","fence","parser_version","status","lease_until","budget_day","reserved_microusd"],"UPDATE":["status","error_code","charged_microusd","finished_at"]},"import_outputs":{"SELECT":["item_id","fence","content","created_at"],"INSERT":["item_id","fence","content"]},"import_audit":{"SELECT":["id","batch_id","item_id","event","created_at"],"INSERT":["id","batch_id","item_id","event"]},"import_daily_usage":{"SELECT":["day","reserved_microusd","charged_microusd"],"INSERT":["day"],"UPDATE":["reserved_microusd","charged_microusd"]}}'::jsonb;
BEGIN
  -- Re-read role attributes and memberships after GRANT. No role mutation is
  -- allowed to bypass the commit gate; this does not prevent later drift.
  SELECT * INTO target_role FROM pg_roles WHERE oid=target;
  ...
  -- Exact pinned direct column ACLs; missing, extra and future grants fail closed.
  IF EXISTS(SELECT 1 FROM pg_class c CROSS JOIN LATERAL aclexplode(c.relacl) a WHERE a.grantee=target)
    OR (SELECT count(*) FROM pg_attribute c CROSS JOIN LATERAL aclexplode(c.attacl) a WHERE a.grantee=target)<>90
    OR EXISTS(SELECT 1 FROM pg_attribute col JOIN pg_class c ON c.oid=col.attrelid JOIN pg_namespace n ON n.oid=c.relnamespace
      CROSS JOIN LATERAL aclexplode(col.attacl) a WHERE a.grantee=target AND
      (n.nspname<>'public' OR c.relkind<>'r' OR col.attnum<=0 OR col.attisdropped
        OR a.grantor<>c.relowner OR a.is_grantable
