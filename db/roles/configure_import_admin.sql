...
-- must pre-create EMPTY hzense_import_admin LOGIN NOINHERIT CONNECTION LIMIT 2.
...
-- creator. Accept only Neon's exact neondb_owner -> hzense_import_admin management
-- edge, granted by cloud_admin with neither INHERIT nor SET. It gives the import
...
BEGIN
  ...
  END IF;
  SELECT * INTO target FROM pg_roles WHERE rolname='hzense_import_admin';
  IF NOT FOUND OR NOT target.rolcanlogin OR target.rolinherit OR target.rolconnlimit<>2
    ...
    RAISE EXCEPTION 'Pre-create a restricted hzense_import_admin LOGIN NOINHERIT CONNECTION LIMIT 2';
  END IF;
  IF (SELECT count(*) FROM pg_catalog.pg_auth_members WHERE member=target.oid OR roleid=target.oid)>1
    ...
    RAISE EXCEPTION 'Import administrator must have no ownership, unsafe memberships, settings or existing direct ACLs';
  END IF;
  ...
  IF (SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
    WHERE n.nspname='public' AND c.relkind='r' AND c.relowner=database_owner
      AND c.relname IN ('import_batches','import_items','import_documents','import_attempts','import_outputs','import_audit','import_daily_usage'))<>7 THEN
    RAISE EXCEPTION 'Verify seven owner-controlled import tables before provisioning';
  END IF;
  EXECUTE format('GRANT CONNECT ON DATABASE %I TO hzense_import_admin',current_database());
END;
$import_admin$;
GRANT USAGE ON SCHEMA public TO hzense_import_admin;
GRANT SELECT(id,owner_id,fingerprint,intent,configuration,cancelled,created_at,deleted_at),INSERT(id,owner_id,fingerprint,intent,configuration),UPDATE(cancelled,deleted_at) ON public.import_batches TO hzense_import_admin;
