...
-- Creates only a new login: no generation table grants and no existing password change.
...
BEGIN
  ...
  END IF;
  IF EXISTS(SELECT 1 FROM pg_roles WHERE rolname='hzense_generation_admin') THEN
    RAISE EXCEPTION 'Generation role already exists; stop without rotating or overwriting its password';
  END IF;
END;
$guard$;
CREATE TEMP TABLE generation_setup_result(password text) ON COMMIT DROP;
DO $create$
DECLARE
  new_password text := encode(sha256(convert_to(gen_random_uuid()::text || gen_random_uuid()::text || gen_random_uuid()::text,'UTF8')),'hex');
  target oid;
BEGIN
  EXECUTE format('CREATE ROLE hzense_generation_admin LOGIN NOINHERIT NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS CONNECTION LIMIT 2 PASSWORD %L',new_password);
  target := 'hzense_generation_admin'::regrole;
  IF (SELECT count(*) FROM pg_auth_members WHERE member=target OR roleid=target)<>1
    ...
    RAISE EXCEPTION 'Unexpected Generation role membership; credential creation rolled back';
  END IF;
  INSERT INTO pg_temp.generation_setup_result VALUES (new_password);
END;
$create$;
-- In Neon SQL Editor, open this SELECT result tab after COMMIT succeeds.
-- Build the approved pooled HZENSE_GENERATION_DATABASE_URL with this password.
SELECT password AS "HZENSE_GENERATION_DATABASE_PASSWORD - SECRET" FROM pg_temp.generation_setup_result;
COMMIT;
