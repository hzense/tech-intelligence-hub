...
As of 2026-09-03, the repository reports `can_approve_pull_request_reviews=false` and `CONTINUOUS_DAILY_PUBLISH_ENABLED` is absent. The organization-level cause could not be independently inspected with the current account and must be rechecked by an organization administrator before enabling publication. The latest observed scheduled runs on 2026-09-02 completed as zero-candidate no-ops, so they created no artifact, branch or Draft PR. Scheduled and manual runs can generate, validate and retain an artifact when eligible Signals exist, but the publication jobs remain gated. The 2026-08-20 dry-run has been independently verified through its manifest and SHA-256.
