...
文本类资料合并相邻短段，累计达到约 800 个 Unicode 码点后在下一段前结束，单片段最多 1600 个码点。超过上限的自然段优先在后半段的句末、分号或换行处切分；没有适合边界时硬切，不截掉尾部、不重复插入重叠文本。该长度不是 token 预算，AI 输入总量限制仍独立生效。

Markdown ATX 标题、HTML h1–h6、DOCX Heading／Title 样式开启新组，短标题可与后续正文合并；Markdown 围栏代码里的标题符号不作为章节边界。HTML 在块结束处也分隔，行内标签与 br 不再制造大量碎片。PDF 在单页内分段，绝不跨页合并；CSV／XLSX 继续保留独立单元格，不参与合并。

`locator.paragraph` 表示合并后的起始原段号；`region` 记录段落范围，或长段切片在该段去除首尾空白后的码点区间（从 1 开始，含结束位置）。HTML 段号对应提取出的正文块，不是 DOM 行号。此实现不推断文章语义、不提供 OCR，也不保证识别所有自定义标题样式。

解析源码参与现有 parser version 哈希，因此新版本仅用于匹配的新批次，不改写已有 `import_outputs`、证据引用或历史去重结果；旧版排队任务不会静默换解析器。重新导入同一资料的片段结构可能不同，不能依靠旧片段哈希保证跨版本去重。上线前须核对旧队列；本算法更新本身不触发重解析、模型调用或发布。
...
管理页以文件名或链接为任务标题，批次编号保留为辅助信息。解析片段（含定位信息）完全相同的资料，在同一用户的未取消、未删除任务中按创建时间和编号保留一份展示；重复项默认折叠，可勾选“显示重复资料”查看并删除。不会仅按文件名判断，不跨用户去重，也不会改写解析结果。生成资料选择先筛选非重复资料再分页，因此旧的保留项不会被较新的重复项挤出列表。删除保留项后，剩余同内容资料自动成为可选项。

“删除任务”经确认后设置 `deleted_at`，从列表和生成输入选择移除，保留解析、审计、费用与请求幂等记录；这不是彻底清除所有数据库数据。未执行项随删除取消；正在执行、结果未知或取消后执行租约未过期的任务暂不能删除。删除继续调用原件清理，清理失败会显示尚待清理，后台保留兜底处理。已删除资料的普通详情和解析输出不可读取。

上线需要迁移 `0017_import_task_visibility.sql` 并重配导入角色的精确列权限（仅新增 `deleted_at` 的 SELECT/UPDATE）；新代码需在迁移与授权完成后部署。本地测试不代表生产已应用。
