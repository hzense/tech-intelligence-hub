...
**原件不归档。** 上传原件和链接抓取原文仅在私有 Blob 临时存放；Worker 提交成功、解析失败或提交结果不确定后，均在 `finally` 中按精确对象路径及 ETag 条件删除。删除失败不覆盖解析状态或重试数据库提交，返回 `original_cleanup=pending` 并输出固定脱敏告警 `import_original_cleanup_pending`；页面不得宣称已删除。取消批次同样触发清理，取消／处理完成后的迟到上传回调也清理对应对象。Sandbox 在解析后停止，异常中断另有平台超时兜底。
