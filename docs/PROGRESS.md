...
**生成等待延长（2026-09-19，本地修改）：** 按用户要求采用 5 分钟接口时限，模型截止改为 285 秒，预留 15 秒供校验和保存；原 7 分钟租约不变。页面说明区分当前时限与历史超时任务，不改预算、模型、生产开关或旧记录；部署须确认 Fluid Compute。尚未提交或部署，不代表生产生效，也未重试未知任务。
