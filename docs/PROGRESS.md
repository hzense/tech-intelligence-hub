...
组织类型增量验证：Web 默认回归 660 项通过／11 项环境测试跳过；实际补证组件合成浏览器回归 1 项通过，覆盖类型／证据选择、重新勾选、刷新回读及不增加 AI 调用。PR #162 复审后将自动规则收紧为整条引文的独立受控肯定句，补齐逗号后置限定、假设、被否认主张、跨行和问句拒绝测试，其他表述保留人工入口。人工确认及材料计划统一支持 24 个组织和 12 个人物，新增完整准备流程与 Unicode 请求体边界测试，数据库材料契约／核验／执行器单元测试 139 项通过。服务端隔离测试覆盖会话归属、关闭门禁、材料变化拒绝、确认随提案持久化和同输入指纹稳定；类型检查、改动 lint、格式检查、后台执行器打包及生产构建通过。未连接生产数据库、调用真实 AI 或部署。
