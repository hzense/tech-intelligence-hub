...
组织类型增量验证：Web 默认回归 653 项通过／11 项环境测试跳过；实际补证组件合成浏览器回归 1 项通过，覆盖类型／证据选择、重新勾选、刷新回读及不增加 AI 调用。PR #162 评审后将自动规则收紧为句首主体的受控肯定句式，补齐假设、被否认主张、跨行假设和问句拒绝测试，其他表述保留人工入口。服务端隔离测试覆盖会话归属、关闭门禁、材料变化拒绝、确认随提案持久化和同输入指纹稳定；类型检查、改动 lint、格式检查、后台执行器打包及生产构建通过。未连接生产数据库、调用真实 AI 或部署。
