...
**AI 长任务与执行进度（2026-09-19，本地开发，未部署）：** 将同步生成改为 Workflow 排队，API 返回 202，后台单次模型截止 25 分钟、Step 30 分钟、数据库租约 32 分钟。列表／详情显示阶段进度、等待动画与已执行时间，每 5 秒只读更新；不是模型 token 百分比。保留每批 5／每日 10 美元的既有配置约束、500 字摘要／最多 5 条／Profile 8192 输出额度、原任务幂等、结果校验和无自动付费重试。不切换公开数据源、不发布。生产仍需 0019 迁移、三列最小授权、Workflow／Fluid／1800 秒资格核验及显式开启新门禁后，另做一次受控验收；不把本地构建或配置保存算作上线。详见 [AI 信号候选生成](AI_SIGNAL_GENERATION.md#后台执行与阶段进度0019待生产启用)。

**长任务本地验证：** Web 默认测试 467 通过／8 环境项跳过；数据库默认测试 2064 通过／644 环境项跳过；独立 PostgreSQL 18.4 的生成存储、角色权限与历史升级回归 73 通过；合成浏览器 22 通过（含移动宽度详情、阶段自动刷新、完成后停止轮询）；构建后鉴权／HTTP 23 通过／1 可选浏览器项跳过。生产构建、数据库类型检查、Web／数据库 lint、全仓格式和冻结锁文件安装通过；修补 Workflow 间接依赖后生产依赖审计为零已知漏洞。所有新增执行测试使用本地替身或隔离数据库，未外发用户资料、未调用真实 AI，不代表生产长任务已验收。
