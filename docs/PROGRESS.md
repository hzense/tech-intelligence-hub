...
**最后更新：** 2026-09-12
**当前阶段：** 网站 MVP 已验收；FTS-1 数据库搜索已完成生产切换与功能验收，当前推进 AI 自主 Signal 设计整合，功能开发待启动；持续内容运营与运维收尾并行
...
网站页面、工程基础和既定 MVP 已完成，剩余工作主要集中在持续获取信息、组织知识和生成可靠回答。上表 **98% 仅沿用此前 MVP 生产运维口径**，不表示完整平台只剩 2%，FTS-1 的已上线状态由下方 2026-09-11 生产切换证据独立支持。
...
- [ ] 实跑并验证自动 Draft PR；开关已启用不代表已经生成候选或发布内容。
...
2026-09-12 补充核对：main 已有 [2026-09-12 Daily](../content/daily/2026/2026-09-12.md)，状态为 published/live，生成版本 daily-v2。上述本地生成、尚未发布等日期记录保留其历史范围，不代表当前没有正式日报；本次未重新核验生产部署或持续定时运行。

完成标准：有连续发布的真实内容与可追溯记录；单篇正式日报不代表稳定日常更新或无人审核生产已经完成。

### 3. AI 自主 Signal（当前优先阶段）

**2026-09-12：设计已完成并提交 PR #64，正在整合主设计与看板；尚未合并，功能尚未实施。** [主设计](DESIGN.md) 是产品入口，[专项设计](AUTONOMOUS_SIGNAL_PIPELINE.md) 保存详细契约。用户要求自动采集、AI 研判、自动入库发表，不设置日常必经人工审核。

- [x] 完成总体与专项设计：网页 AI API 配置、三类导入、手动/定时任务、无人审核发布、数据权威迁移及五阶段验收。
- [ ] 完成 PR #64 评审与合并。
- [ ] A：管理员认证、AI 连接和模型/提示词配置、密钥保护。
- [ ] B：来源增量采集、文件/OCR、批量链接、统一队列与手动运行。
- [ ] C：去重、分类、实体匹配、证据核验、中文研判及自动发布规则。
- [ ] D：Signal 数据库迁移、公开版本、Outbox、搜索和缓存、更正撤回。
- [ ] E：时区定时任务、暂停/停止、重试、预算、监控与自动更正实跑。
- [ ] 三类入口的真实无人审核端到端生产验收。

完成标准：证据不足自动补查或暂缓；合格内容无需逐条人工批准即可公开，证据/配置/模型可追溯，重复运行不重复发表。当前 Seed 和 Daily 人工门禁尚未改变；Daily 自动发表另行实施。此次文档工作不增加网站或完整平台的完成百分比，55%–65% 保留为此前粗估，新范围完成拆分后再重新估算。
...
FTS-1 已完成生产切换。当前优先 **PR #64 设计整合与评审 → AI 自主 Signal 的 A–E 实施 → 持续自动内容运营 → 混合搜索 / RAG**。时间线与图谱可在实体和 Signal 数据积累后分批交付；其余运维收尾持续跟进。恢复未验证风险保持记录，不重启操作者已取消的演练。
...
2026-09-12：PR #64 整合 DESIGN 与 AI 自主 Signal 专项方案，更新 A–E 待办和自动发布原则；核对 main 正式 Daily 文件，区分历史检查点与当前内容。仅设计文档变更，未执行应用开发、迁移或部署。
