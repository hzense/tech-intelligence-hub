...
**能力测试时效调整（2026-09-20，本地完成，未提交／部署）：** 按用户确认，取消成功能力测试超过 24 小时自动阻断生成的规则，改为配置页与生成页的非阻断提醒。资格仍绑定当前启用连接、有效密钥、精确连接修订与模型；缺少基础／结构化测试或要求的工具测试仍阻断，未来时间、未完成和非成功记录不能作为证明。刷新匹配的成功测试后清除对应提醒，不自动测试或调用模型。预算、任务锁、去重、来源与结果校验不变，无数据库迁移或生产配置变更。

本批验证：数据库默认测试 2072 通过／648 环境项跳过，Web 默认测试 470 通过／8 环境项跳过；另行启用实际组件合成浏览器测试 45 通过，独立 PostgreSQL 18.4 + pgvector 0.8.6 的 AI 角色／存储集成 104 通过，覆盖旧证明可保存／读取／执行、提醒清除、模型不匹配及未来／未完成证明拒绝。Web／数据库类型检查、lint、生产构建和改动文件格式检查通过。所有模型响应为测试替身，未调用真实 AI；本地测试通过不代表线上已更新或真实候选生成成功。

**生成请求恢复修复（2026-09-20，本地验证完成，生产待验收）：** 修复明确创建拒绝后刷新丢失恢复入口的问题；拒绝原因及前序任务编号绑定原请求保存在当前标签页，刷新后可安全核对放弃或显式重新生成，外发同意不持久化。放弃仍须新鲜只读 `not_found`，重新创建前撤销旧拒绝记录；网络／提交未知、存储故障、错配记录不能解锁。旧请求没有拒绝记录时提供原编号确认的操作指引，不绕过去重、不自动调用 AI。无数据库迁移、权限、预算或生产开关变更，生产任务未重跑。

本批验证：实际组件合成浏览器测试 27 通过，覆盖刷新恢复、前序编号绑定、旧会话兼容、404 不自动解锁、损坏／错配恢复记录、存储故障、提交未知后撤销恢复入口和迟到任务核对；Web 默认测试 470 通过／8 环境项跳过，Web 类型及 lint、改动文件格式检查通过。所有生成响应为本地替身，不代表生产生成验收或正式域名已经切换。

**候选标题扩容（2026-09-19，本地修改，待部署）：** 按要求将标题上限由 50 调整为 80 个 Unicode 码点，同步 Schema、系统提示、服务端校验、页面错误说明及 80／81 边界测试。摘要 500、单次最多 5 条、输出上限 8192 token 不变；历史验收记录保留当时规则，不改写或重跑旧任务。

**逐条候选校验修复（2026-09-19，本地开发，未部署）：** 针对整批因一条标题超长／未知日期仍附证据而失败的问题，补明确日期提示规则；SDK 信封解析与逐条业务校验分离，保留合格的私有待审核候选，被拒绝项仅保存原始序号和固定字段错误，不保存其原始正文。全部拒绝仍为失败并保留诊断和费用，部分通过在结果页显示数量，不代表审核或公开发布。使用既有 JSONB 字段，无迁移、权限或预算变更；旧失败任务不改写，不自动重试，也未调用真实模型。详见 [AI 生成说明](AI_SIGNAL_GENERATION.md#逐条候选校验2026-09-19本地修复待部署)。

本批验证：Web 默认测试 470 通过／8 跳过，Ingestion 56 通过／1 跳过；另行开启合成浏览器回归 23 通过，生成存储单测 12 通过，独立本机 PostgreSQL 18.4 生成存储集成 28 通过。PR #124 评审后补齐保存层诊断精确结构、字段／原因码白名单、索引和状态校验；存在拒绝项时原因改为固定服务端说明，合格候选绑定任务原文快照重新执行完整合约校验。覆盖原始文本／额外字段／非法原因码拒绝、嵌套候选字段和伪造引用拒绝及混合、零候选保存。覆盖合格／拒绝混合、全部拒绝、字段错误、旧结果读取、私有文本转义、费用不减免和不重复调用；Web／Ingestion／Database 类型及 lint、生产构建通过。隔离测试数据库服务已停止；未使用生产凭据或操作线上任务。

**AI 长任务与执行进度（2026-09-19，PR #122，已生产启用、真实生成待验收）：** 原 1800 秒 Functions 方案被当前套餐拒绝，按用户要求保留套餐，改为 300 秒以内 Workflow 短步骤派发／轮询非持久 Sandbox。Sandbox 最长 30 分钟、单次模型截止 25 分钟、数据库租约 32 分钟；结果直接落库。列表／详情每 5 秒只读更新阶段与耗时，不伪装 token 百分比。保留每批 5／每日 10 美元、500 字摘要／最多 5 条／Profile 8192 输出额度、幂等与无自动付费重试。经独立审批，生产 0019 迁移、全量核验及六项最小列授权已完成，显式列权限合约为 51。新备份存在但恢复未演练；Hobby 套餐未变，CLI 无 AI Sandbox 检查通过并已停止测试实例。长任务开关已启用，新部署已提升至 hzense.com，管理员六项只读预检通过。生产 Workflow 投递／运行时 OIDC／真实模型及候选落库仍待一次另行授权的端到端验收，本次未调用模型或重试历史任务。详见 [生产启用记录](production-evidence/2026-09-19-generation-progress.md)。

**长任务本地验证：** 当前套餐改造后 Web 默认测试 468 通过／8 环境项跳过；数据库默认测试 2067 通过／645 环境项跳过；独立 PostgreSQL 18.4 的生成存储、角色权限与历史升级回归 74 通过，包含实际旧 Schema 只读查询、部分授权拒绝与新授权事务回滚。生产构建、Worker 非公开产物追踪、数据库类型检查、Web／数据库 lint、全仓格式通过。前一版本合成浏览器 22 通过（含移动宽度详情、阶段自动刷新、完成后停止轮询）；构建后鉴权／HTTP 23 通过／1 可选浏览器项跳过，本轮未重跑这些浏览器验收。修补 Workflow 间接依赖后生产依赖审计为零已知漏洞。所有新增执行测试使用本地替身或隔离数据库，未外发用户资料、未调用真实 AI，不代表生产长任务已验收。
...
- [x] 2026-09-19 按操作者要求永久停用 Continuous Daily candidate：删除定时及手动触发工作流、专用写权限白名单和 `CONTINUOUS_DAILY_PUBLISH_ENABLED` 仓库变量。现有日报内容及历史运行记录保留。
