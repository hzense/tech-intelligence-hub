...
**最后更新：** 2026-09-13
**当前阶段：** 旧网站 MVP 已交付；Signal-first v2 数据底座 V2-1a 已合并，V2-1b 任职与证据首批本地实现完成，应用和生产尚未按新版切换
...
- [ ] V2-1 数据关系：版本化 Signal、多来源证据、人物／组织任职、发表事务与历史导入。
  ...
  - [ ] V2-1b 任职关系及证据、稳定事件键、不可变保护、受限角色与发表／撤回事务、Outbox。
    - [x] 首批任职与证据本地实现：复用 `relations` 的身份／方向／日期，追加 `0005` 两张私有表、类型化复合外键、保守按日区间判断与证据状态预检；见[实施契约](PERSON_ORGANIZATION_AFFILIATIONS.md)。整仓 979 项测试、build／lint／typecheck／format 通过；53 项原生 PostgreSQL 集成测试本地跳过，PGlite 补充核验 23 表及 45 项非法 SQL 拒绝通过。本批通过 PR 交付，远端 CI 与评审待确认；未执行生产迁移，不代表 V2-1b 全部完成或资源页面已上线。
