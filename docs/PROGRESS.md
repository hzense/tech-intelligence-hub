...
**AI 长任务与执行进度（2026-09-19，PR #122，未生产启用）：** 原 1800 秒 Functions 方案被当前套餐拒绝，按用户要求保留套餐，改为 300 秒以内 Workflow 短步骤派发／轮询非持久 Sandbox。Sandbox 最长 30 分钟、单次模型截止 25 分钟、数据库租约 32 分钟；结果直接落库。列表／详情每 5 秒只读更新阶段与耗时，不伪装 token 百分比。保留每批 5／每日 10 美元、500 字摘要／最多 5 条／Profile 8192 输出额度、幂等与无自动付费重试。新增旧 Schema 历史只读兼容、固定 0019 维护策略及原子六项列权限升级脚本。生产仍需独立迁移／授权、Sandbox 用量与 OIDC 核验、显式启用及受控验收；本轮未执行生产写入或真实 AI 调用。详见 [AI 信号候选生成](AI_SIGNAL_GENERATION.md#后台执行与阶段进度0019待生产启用)。
