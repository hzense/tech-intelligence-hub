...
本批验证：Web 默认测试 470 通过／8 跳过，Ingestion 55 通过／1 跳过；另行开启合成浏览器回归 23 通过，生成存储单测 12 通过，独立本机 PostgreSQL 18.4 生成存储集成 27 通过。PR #124 评审后补齐保存层诊断精确结构、字段／原因码白名单、索引和状态校验，覆盖原始文本／额外字段／非法原因码拒绝及混合、零候选保存。覆盖合格／拒绝混合、全部拒绝、字段错误、旧结果读取、私有文本转义、费用不减免和不重复调用；Web／Ingestion／Database 类型及 lint、生产构建通过。隔离测试数据库服务已停止；未使用生产凭据或操作线上任务。
