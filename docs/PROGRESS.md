...
**最后更新：** 2026-09-13
**当前阶段：** 旧网站 MVP 已交付；Signal-first v2 私有资格发表核心与候选核验已合并；Google 管理员认证已上线，指定账号登录／退出证据已归档；受限公开发布、独立撤回和网站／搜索统一实时读取已完成本地开发与隔离验证，待 PR 交付；生产数据库及网站尚未切换新版
...
- [ ] V2-1 数据关系：版本化 Signal、多来源证据、人物／组织任职、发表事务与历史导入。
  ...
  - [ ] V2-1b 任职关系及证据、稳定事件键、不可变保护、受限角色与发表／撤回事务、Outbox。
    ...
    - [ ] 后续交付：本批 PR 评审与 CI → 生产迁移及 Publisher／Reader 最小权限兼容审计 → 内容就绪后切换并做真实发布／搜索／撤回验收；再完成历史导入和数据权威迁移。认证剩余验收见 V2-2；原文核验执行器、AI 任务和完整业务后台仍待接入，记录的 verified、私有 published 或历史回执均不是可公开许可。
