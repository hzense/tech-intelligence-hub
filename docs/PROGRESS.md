...
**四项信息人工确认发布（2026-09-25，本地开发，未提交／未上线）：** 按最新确认，以日期、组织、人物、领域作为新发布通道的业务完备条件；缺项可手填，管理员一次确认直接公开，不再要求独立签名核验。采用独立追加修订、公开视图与专用读写角色，不篡改旧核验记录、原 AI 候选或费用；列表／详情／搜索实时读取并支持撤回。新增 0025 迁移和默认关闭的配置，生产迁移、ACL、环境和内容验收尚未执行。详见 [人工确认发布](EDITORIAL_PUBLICATION.md)。下方旧批次的六项审核、签名门禁描述为历史方案，不再是本通道的发布条件。

本批验证：Web 677 通过／12 环境项跳过；候选相关合成浏览器 3 通过；数据库普通套件 2330 通过，隔离 PostgreSQL 的持久化／ACL／并发及 fresh／upgrade 迁移分组通过（26 migrations／58 tables）。类型、改动 lint、格式及开关开／关生产构建通过，Turbo 开关缓存失效已验证。复审的旧回执误报、失效领域不可删除、请求挂起三项均已修复。当前未提交、无生产变更；0025 尚需独立线上维护审批范围，不可借用旧 0024 批准执行。

**组织类型规则与人工确认（2026-09-25，本地开发，未提交／未上线）：** 扩展公司／机构的显式自然语言类型识别，保留跨组织、否定及无依据推断的拒绝规则。在补证资料包“准备核验材料”后，为未登记且缺类型提案的组织提供类型、已存公开原文片段和明确确认入口，不需填写 ID／JSON，不调用 AI。服务端复算 owner／资料包／候选／目录指纹，拒绝伪造、过期或跨请求证据；人工确认随完整材料提案追加保存并可刷新查看，仍须六项审核、独立核验、登记和发布。无需新迁移或 ACL；本批未操作生产任务。详见 [组织类型人工确认](MATERIAL_REGISTRATION.md#组织类型规则与手动确认2026-09-25本地开发未上线)。

组织类型增量验证：Web 默认回归 661 项通过／11 项环境测试跳过；实际补证组件合成浏览器回归 1 项通过，覆盖类型／证据选择、重新勾选、刷新回读及不增加 AI 调用。PR #162 复审后将自动规则收紧为整条引文的独立受控肯定句，补齐逗号后置限定、假设、被否认主张、跨行和问句拒绝测试，其他表述保留人工入口。人工确认及材料计划统一支持 24 个组织和 12 个人物，总证据上限 256 条、总计划仍限 240 KiB；新增 49 个独立片段的完整准备与核验流程、Unicode 请求体边界测试，数据库材料契约／核验／执行器单元测试 139 项通过。服务端隔离测试覆盖会话归属、关闭门禁、材料变化拒绝、确认随提案持久化和同输入指纹稳定；类型检查、改动 lint、格式检查、后台执行器打包及生产构建通过。未连接生产数据库、调用真实 AI 或部署。

上一批验证：Web 默认回归 604 项通过／11 项环境测试跳过；解析与生成契约 82 项通过／1 项环境测试跳过；补证浏览器回归 1 项、相关数据库单元测试 81 项通过；Web 类型检查、改动 lint、格式检查和后台执行器打包通过。合成闭环覆盖补充原文 → 私有补全 → 人工确认材料 → 模拟独立核验，不代表生产付费验收。

**补证资料驱动补全（2026-09-25，本地开发，未提交／未上线）：** PR #159 容量修复已合并为 `affc3a1` 并部署到 Production，实际补证请求已保存 81 个片段；当前业务阻点是原补全不读取补充资料、规则要求既有组织及领域字面命中。本批增加每个补证包的显式计费补全入口，复用既有任务表、预算、Workflow、Sandbox；固定“原候选引用片段＋全部补充片段”快照且保持 48,000 字节 AI 输入上限。新结果可提出有原文依据的人物、组织类型和启用领域，以及对应原主张的新引用；准备材料时再次校验后仍需人工确认、独立复查和签名，不自动登记或发布。本批未修改生产配置、ACL、Schema，未调用生产 AI；生产闭环待上线后另行验收。下方均为各批次历史快照。

**通用补证启用准备续接（2026-09-24，本批未提交／未上线）：** 基础 [PR #157](https://github.com/hzense/tech-intelligence-hub/pull/157) 已合并为 `c3ed01e`，Production 已部署，但生产通用补证未启用。用户同意补齐独立执行器与迁移门禁，并选择“规则／既有 AI 补全准备材料 → 页面人工确认六项 → 独立 GitHub 执行器重抓原句并签名 → 网站登记”，不要求人工填写 JSON。本批新增未可信提案／不可变人工确认两表（0024）、精确请求与确认双 UUID 派发、默认关闭的 review／executor 配置，以及独立 0023–0024 风险迁移门禁。代码目标为 25 迁移／57 表，不代表生产已迁移。旧 main 的只读 [preflight 36022413674](https://github.com/hzense/tech-intelligence-hub/actions/runs/36022413674) 成功，彼时只待 0023；本批合并后预期待 0023、0024，须重新预检、绑定新备份并分别审批 DDL、两角色最小增量 ACL、签名与派发配置。无生产写入、AI 调用或 Signal 发布；真实闭环待验收。详见[本批准备证据](production-evidence/2026-09-24-material-review-preparation.md)。下条“本地未部署”保留为 PR #157 提交前快照。

**通用补证与登记（2026-09-24，本地开发，未部署）：** 候选页新增“通用补证与登记”，选择本人已解析公开链接后建立独立快照；提供签名材料计划接入、专用登记/核验角色、追加式请求/报告/回执，以及登记后重查发布材料。原候选与账本不变，不自动调用 AI 或发布。0023 迁移待批准执行。独立核验服务的实际调查与签名执行尚未接入，不代表自动补全生产闭环。详见 [通用材料登记](MATERIAL_REGISTRATION.md)。

**正式发布材料包（2026-09-24，开发交付范围，部署另验）：** 候选页集中呈现日期依据、人物/组织实际关联、领域、主张与公开来源，并保留缺项和消歧要求；管理员仍只确认，不填写 ID 或 JSON。确认绑定服务器材料指纹，服务端复算并在审核备注中保存指纹；材料变化必须刷新后重新确认，未知提交重试保留原编号和修订。公开证据匹配校验来源域名，查询截断、补全读取失败或损坏不再静默降级。CI 显式执行候选页面与隔离 PostgreSQL 查询回归。无新增迁移/权限、AI 调用或生产发布。缺失正式实体登记与公开证据/许可核验执行器仍待开发，不能将本批材料展示算作发布闭环完成。详见 [正式发布材料包](CANDIDATE_REVIEW.md#正式发布材料包2026-09-24)。

**补全单次真实验收完成（2026-09-23 19:06 UTC）：** 经用户另行授权，GPT6 r1 核验阶段完成唯一一条私有补全任务 `3e7a8de7`。API 202、Workflow 派发、数据库 `completed` 与重新打开页面后的结果一致；执行 5.385 秒，预留 0.404935 美元、系统记账 0.189080 美元（未独立对账供应商最终账单）。标题／摘要／主张保持不变，引用匹配保存原文；没有人物依据时保留为空，没有自动发布或重试。本轮证明执行和持久化链路可用，不代表新增人物正例、正式实体与公开证据齐备或发布验收通过。详见 [单次验收记录](production-evidence/2026-09-23-candidate-enrichment.md#单次真实补全验收2026-09-23-1906-utc)。以下“真实调用待验收”段落为此前阶段快照。

**补全最小授权已完成（2026-09-23）：** 用户确认后已在生产执行 `upgrade_candidate_enrichment.sql` 并提交事务。独立只读核对 generation 角色共 96 项精确列授权：补全私表新增 SELECT 23 / INSERT 10 / UPDATE 12，原生成表 51 项保持不变；无表级权限、无转授权。正式网站六项只读连接预检全部通过，候选审核页正常读取并显示“启动 AI 自动补全”。本次未创建任务、调用 AI、改密码或发布；补全任务数仍为 0。真实 Workflow 派发、模型返回、提案落库及费用记账待单次另行授权验收。下方“待授权”内容为此前阶段快照，详见 [生产记录](production-evidence/2026-09-23-candidate-enrichment.md)。

**补全生产准备最新核验（2026-09-23）：** 核验器修正 PR #154 已合并为 `f748a56`，PR/main CI 通过，本地 main 已同步，Vercel Production READY 并绑定正式域名。用户审批的只读 `verify` run `35883128941` 成功：23 项迁移、52 张表。Neon 独立核对补全表已存在，但 generation 角色仍仅有原 51 项列授权；下一步是经确认后执行补全私表的 45 项最小列权限升级，再做无 AI 线上可用性检查。尚未执行本次授权、付费补全或发布；下方本地修正待提交的描述为此前阶段快照。

**候选 AI 补全执行器（2026-09-23，代码已上线，生产数据准备中）：** PR #152 已合并为 `f530f70`，PR/main CI 通过；Vercel Production 已 READY，生产候选页已显示补全入口，数据库健康检查正常。候选页可对确实缺少日期或人物的候选显式启动私有补全任务；复用 Profile `verify` 阶段、生成预算、Workflow 和隔离 Sandbox，模型调用步骤不自动重试。结果只包含有逐字原文证据的日期、人物和组织提案，标题／摘要／主张保持锁定；不创建正式实体、公开证据、核验或发布。新增独立任务表 `0022_candidate_enrichment_runs.sql`、最小列权限升级脚本、任务进度／费用列表及补全后正式目录重新检查。用户已要求启用；`0022` 已落库，角色授权与真实模型调用尚未执行。迁移后及独立 Schema 核验失败，根因是核验器遗漏既有 `hzense_candidate_verifier` 的依赖锁函数授权；本地已修正精确允许列表并补齐实际授权后的完整 Schema 回归，尚未提交或部署。下一步合并修正后只重跑 `verify`，不重跑迁移；核验通过后再升级补全列权限。入口出现不代表任务可用。

**当前权威状态（2026-09-22）：** `main` 为 `f8278bb`，PR #130–#147 已合并，PR #147 对应代码已部署到正式域名；当前无未合并 PR。任务内确认式候选审核、追加式修订、转换、签名核验接入、组装、发布和撤回代码均已交付，但这只是应用代码就绪。生产数据库已知迁移到 `0019`；`0020_candidate_reviews.sql` 与 `0021_candidate_review_attestations.sql`、候选审核专用角色、公钥和发布授权尚未在生产启用，因此页面不得宣称可正式发布。当前按依赖推进：先建立 `0020–0021` 独立生产迁移门禁与只读预检，再配置最小权限角色，随后开发可信核验执行器并完成单条生产发布/撤回验收，最后切换 Signal-first 公开页面与自动采集。下方标注“本地开发/未部署”的段落是各批次历史快照，若与本段冲突，以本段及对应 PR/生产证据为准。

**确认式发布准备（2026-09-21，本地开发，未提交／部署）：** 补齐首个审核修订及撤回后新修订的服务器准备与确认入口。系统从当前私有候选、正式人物／组织、启用的领域分类和已核验公开证据确定性生成审核快照与稳定事件键；全部唯一匹配后仅显示“确认候选并送核验”，浏览器不提交标题、摘要、实体、证据、领域或事件键，旧人工保存动作已从 API 关闭。缺失／歧义时逐项阻断，私有引用不会自动提升为公开证据；确认仍不等于独立事实核验或公开发布。既有转换、签名核验、组装、发布和撤回门禁保持不变。

**候选审核到发布适配（2026-09-21，本批代码交付，未生产启用）：** 在原任务逐条材料页增加人工审核与编辑、追加式修订、正式候选转换及事件去重、独立签名核验报告接入、受控组装/发布/撤回。新增 `0020–0021` 和专用最小权限角色；原生成结果、费用与公开权限不覆盖。隔离 PostgreSQL 完整迁移、角色权限、发布/撤回与实际搜索查询回归已通过；旧请求重试不会重复发布，新审核不阻断历史状态核对，撤回重发需新修订及核验。生产迁移、角色/公钥/授权配置、公开读模式切换及真实发布验收尚未执行；可信核验方仍须实际核验，不能把报告验签当成自动事实核验已运营。详见 [交付范围与上线清单](CANDIDATE_REVIEW.md)。以下记录保留各批次当时的验证状态，不应直接视为当前生产状态。

**审核入口统一（2026-09-21，本地修改，未提交／部署）：** 移除独立候选审核工作台、后台及任务详情汇总入口和专用列表查询／样式；旧地址鉴权后跳转 AI 生成页。仅保留任务内逐条审核及材料页，返回原任务或生成列表，不删除候选数据，不更改鉴权、核验或发布权限。以下早期汇总入口描述保留为历史记录。

**任务内候选审核入口（2026-09-21，本地修改，未提交／部署）：** 生成任务展开区域和详情页的每条私有候选加入“审核候选”直达按钮，绑定任务 ID 与原始候选序号；独立工作台保留汇总用途。未完成任务、无效序号不显示入口；目标页继续鉴权和复检，不保存审核决定或发布、不调用 AI。

**候选审核与发布第一阶段（2026-09-21，本地开发，未提交／部署）：** 新增 `/admin/signal-review` 审核准备列表和逐条材料页，复用任务归属及只读连接，重新核对保存原文哈希和引用，展示发布缺项与引用上下文。生成关闭时仍可阅读，不调用 AI、不保存审核决定、不写核验记录或发布。完整闭环尚未完成：接下来依次实现审核记录持久化、正式候选转换及实体绑定、可信核验、受控发布与生产验收。详见 [候选审核实施边界](CANDIDATE_REVIEW.md)。

**AI 费用展示与 API 费用优先（2026-09-21，本地开发，未部署）：** OpenRouter 候选生成、连接能力测试优先保存响应 `usage.cost`，缺失时保留原保守估算；合法零费用生效，结构校验失败不丢弃已收到费用。预算预留独立保留，历史估算不回填。AI 生成页新增当前管理员 UTC 当日汇总，包含已删除任务，无列表上限；费用不可读不显示为零。不涉及迁移、授权或真实付费调用。

**生成任务列表直达（2026-09-20，本地开发，未部署）：** 创建或复用任务后自动定位任务列表、重置列表筛选，正常流程移除“当前请求”和手动结束跟踪按钮；终态自动清理浏览器跟踪，不删除任务或费用。仅异常创建保留“请求恢复”。不自动执行 AI，不放宽授权、去重和存储安全保护。

**OpenRouter 多模型兼容（2026-09-20，本地开发，未部署）：** 能力测试与候选生成统一读取模型能力、筛选结构化输出端点、选择支持的低推理档位并省略统一温度；供应商 Schema 使用通用子集，本地字数、数量与原文证据校验不变。输出截断单独诊断，不自动重试。能力测试输出额度与费用预留同步为 2048 token，每日预算不变。验证范围为合成报文回归，尚未开展七家真实模型付费验收。

兼容性验证：Web 默认测试 496 项通过／8 项环境测试跳过，配置及预算单元测试 119 项通过，候选业务契约 19 项通过；Web 类型检查、改动 lint、格式与后台 Worker 打包通过。只读实时目录核对覆盖 OpenAI、Anthropic、Gemini、DeepSeek、Qwen、GLM、Kimi；不支持 Schema 的目录项在生成前拦截。未修改生产配置、未付费调用、未提交 PR 或部署。

**AI 配置无变更保存与测试记录精简（2026-09-20，本地开发，未部署）：** 相同连接字段／密钥保存保持原版本和能力证明，真实变更与过期版本写入仍受原门禁控制；明确测试后无需重复保存连接，新建 Profile 与修订按钮分开命名。连接页面仅显示最近 3 次测试，保留底层费用、审计、模型目录及测试资格记录，不回写历史修订，不调用真实 AI。存储单元 55 项、页面回归 36 项、Web 默认 474 项通过（8 环境项跳过），类型及改动 lint 通过；真实 PostgreSQL 集成测试因未配置环境而跳过，待 CI 核验。

**候选信号阅读与证据辅助展示（2026-09-20，本地开发，未部署）：** 候选采用正式信号式标题、摘要、事件时间和核心要点排版，独立辅助栏展示人物、组织、时间依据及决策核对提示；要点、人物和日期分别展开其原文片段与引用。明确标注待审核／未发布，缺失内容明确提示，不生成额外评分、不保存审核决定或发布。保留拒绝项诊断与完整字段折叠查看。桌面与窄屏合成截图已检查；生成浏览器回归 29 项、Web 默认测试 474 项通过（8 环境项跳过），类型与 lint 检查通过。未调用真实 AI。

**AI 生成任务表格布局（2026-09-20，本地开发，未部署）：** 任务列表改为部署列表式表格，按资料／任务、状态、模型配置、创建时间（UTC）、费用、操作分列；增加已加载记录的名称／编号搜索及状态筛选。终态收起进度条，完整编号、内联候选查看和删除放入“更多操作”，诊断说明折叠，窄屏表格横向滚动。授权、任务恢复、费用及调用逻辑不变，不触发真实 AI。

**信号仅输出结果（2026-09-20，本地验证通过，待提交／部署）：** 提示词禁止在候选字段输出思考过程；OpenRouter 请求排除独立推理内容，模型 `reason` 不再原样保存，页面也不直接展示历史自由文本原因。保留候选正文和证据、逐条校验结果及原费用记录，不关闭模型内部推理，不自动重跑历史任务。Web 默认测试 474 通过／8 环境项跳过，另启浏览器回归 28 通过，生成核心 31 通过；类型、改动 lint、生产构建与格式检查通过。未调用真实 AI。详见 [AI 信号候选生成](AI_SIGNAL_GENERATION.md#结果输出边界2026-09-20本地修复待部署)。

**生产构建缓存根因（2026-09-20）：** PR #128 的部署日志明确为 Web `cache hit`、3/3 任务全部命中缓存，仅耗时 1.253 秒；Web 直接引用 database／ingestion 源码，但缺少工作区依赖声明，数据库修复未进入 Web 构建哈希，因此新部署仍复用旧产物。补齐 Web → database／ingestion、database → ingestion 依赖及锁文件，并以隔离工作区真实 Turbo dry-run 验证仅共享源码改变时 Web 构建／测试哈希失效。该修复需随 PR #129 评审部署后验证，不能仅凭部署 SHA 宣称代码生效。

**生产恢复验收补充（2026-09-20）：** PR #127、#128 已评审、合并并部署，但原编号无 AI 确认仍返回 `request_id_conflict`，暂不宣称恢复成功。只读核对历史指纹、批次、模型／连接配置及预算一致，且请求命中最新部署。补充固定字段名／布尔值的身份差异日志，不输出原文、账号、密钥、字段值或快照；不放宽幂等检查，不改写旧任务与费用，不自动调用 AI。仍待诊断部署后的生产验收。
**当前阶段：** 旧网站 MVP 已交付；生产数据库已知到 `0019`，管理员登录、AI 配置、私有导入和 AI 私有候选生成已上线。候选审核与正式发布代码已部署但生产数据库/角色/授权尚未启用；可信核验执行器、单条生产发布验收、Signal-first 公开网站切换和 AI 自动采集仍未完成。
...
**生产验收补充（2026-09-20）：** PR #127 已通过评审与 CI、合并同步，Production 已 Ready；但原编号无 AI 验收仍返回冲突，因此未宣布恢复成功。只读逐字段核对确认模型阶段、连接设置、端点与修订、配置名称及毫秒精度时间一致；旧预估为 595865 microUSD，当前规则按 17757 字节来源、2709 字节提示词、1044 字节系统规则与 8192 输出额度算得 597150 microUSD。系统日期证据提示词升级使派生估算变化，触发第二处身份冲突。补充仅针对已删除记录的估算兼容：其余身份与预算上限不变、历史指纹有效时只返回 `task_deleted`，不复用或执行旧任务；显式新任务采用当前估算。此补充待独立评审、CI 与生产验收，未调用真实模型、未改生产数据库。

**任务身份兼容修复（2026-09-20，本地验证，待 PR／部署）：** 线上原编号确认返回 `request_id_conflict`；追查发现动态 `profile.readiness` 被计入任务指纹及执行前／后的完整配置比较。现将其从身份比较排除，同时保留实时资格校验。历史指纹需先通过完整旧快照或规范化身份校验，不更新历史行；同资料已删除任务可正确返回 `task_deleted`，恢复既有安全退出和显式重建入口。没有迁移、权限、预算或开关变更，没有调用生产模型或重跑旧任务。验证包括提醒新增／清除、历史快照不改写、同编号／不同编号去重、已删除任务显式重建，以及真实模型／提示词／预算变化和损坏指纹仍拒绝。

截至本次修复前，能力测试时效调整已随 PR #126 合并并部署，线上非阻断提醒已核对；以下该批记录保留当时本地验证状态。新的身份兼容修复仍需独立完成 PR、CI、生产部署和原请求无 AI 恢复验收，不能将页面可选模型等同于真实生成成功。

本批验证：Web 默认测试 472 通过／8 环境项跳过，数据库默认测试 2072 通过／651 环境项跳过；另启隔离 PostgreSQL 生成存储回归 31 通过、实际组件合成浏览器回归 28 通过。Web／数据库类型检查、改动代码 lint、生产构建及格式检查通过。模型调用均为测试替身；线上尚待原编号确认验收，不执行收费生成。
