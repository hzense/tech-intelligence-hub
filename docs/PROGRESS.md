...
**候选 AI 补全执行器（2026-09-23，本地开发，未提交／未部署）：** 候选页可对确实缺少日期或人物的候选显式启动私有补全任务；复用 Profile `verify` 阶段、生成预算、Workflow 和隔离 Sandbox，模型调用步骤不自动重试。结果只包含有逐字原文证据的日期、人物和组织提案，标题／摘要／主张保持锁定；不创建正式实体、公开证据、核验或发布。新增独立任务表 `0022_candidate_enrichment_runs.sql`、最小列权限升级脚本、任务进度／费用列表及补全后正式目录重新检查。尚未执行生产迁移、角色授权、真实模型调用或部署。
