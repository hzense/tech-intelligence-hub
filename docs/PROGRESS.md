...
**最后更新：** 2026-09-12
**当前阶段：** 旧网站 MVP 已交付；进入 Signal-first v2 产品重构设计，应用和生产尚未按新版切换
...
- [x] V2-0 设计稿：四个页面、导航、后台流程、人物证据、评分、数据权威、历史迁移与验收已记录，并与 PR #64 的 `b5a0045` 专项技术契约整合。
...
整合基线为 `main@9477ec9`；该基线 Seed 有 82 条 accepted Signal、19 个 Entity、0 个 Person。人物补证是实际开发与迁移工作，不是已完成。PR #64 的 AI 自主 Signal 设计在提交前核对时仍为 OPEN；本次整合提交追加到其 `codex/autonomous-signal-design` 分支，以 `b5a0045` 专项技术契约为输入，保留自动发布、完整导入、来源／模型／任务机制，移除未来 Daily／Weekly 新增生产路线，并以人物补证／legacy 档案替代旧状态直接映射公开。范围仅为七份设计文档；PR 合并和新提交 CI 结果另行核对，不包含应用代码、数据库、工作流或生产配置变更。
