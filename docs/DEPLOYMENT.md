...
> **2026-09-13 开发边界：** [V2-1a 数据底座](SIGNAL_V3_FOUNDATION.md) 的 `0004_signal_version_foundation.sql` 已合并；[任职增量](PERSON_ORGANIZATION_AFFILIATIONS.md) 继续追加 `0005_person_organization_affiliations.sql`。本轮未执行生产迁移或授予新增业务权限。普通 Web 部署仍使用旧 Seed 读取。要求最新 schema 的维护预检会将未应用的 `0004`／`0005` 视为 pending，不能自动跳过或直接应用来解除阻塞。`0005` 的日期约束影响全部旧 `relations`，上线前需核验遗留日期；不自动清洗。正式迁移需另行审批并复核维护流程对该版本的支持；本批不开放新版自动发表，也不重跑已取消的恢复演练。
...
前置只读 `acl-capture` 的对应审批格式见[线上归档说明](./ONLINE_MAINTENANCE.md#当前仓库-acl-公开归档)：
保留独立双采集与公开授权，不要求尚未生成的 ACL 指纹；采集批准不授权后续写操作。
