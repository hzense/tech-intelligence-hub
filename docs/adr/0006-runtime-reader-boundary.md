...
The repository boundary is merged and CI-verified through PR #32–#35, and PR #36 adds the fail-closed scheduled-health guard plus trigger/cron drift validation. A fresh seven-day Neon branch backup, role/database-ACL inventory, Runtime read-only session default and `neondb` ambient-ACL isolation are complete. The maintenance-only `hzense_migrator` connection limit was raised from 5 to 10 after Neon Tables exhausted the old ceiling and caused `53300`; that operational change does not alter Runtime privileges, the Runtime limit of 20 or the Web pool maximum of 1. The health repository variable is currently absent, so scheduled checks are skipped until the first live acceptance succeeds; manual dispatch remains available during the controlled rollout.

The target `hzense` Runtime ACL was independently checked on 2026-09-01 with two catalog-only queries: effective privileges, direct grant sources, the five-column allowlist and the bounded denylist matched the reviewed contract. The [sanitized evidence](../production-evidence/2026-09-01-runtime-reader-acl.md) does not replace the protected production preflight. The independent Runtime credential, target/reserved-database preflight, Vercel Production variables, redeployment, live health response, real five-column query and safe-log/monitoring verification remain incomplete. Until all of those gates pass, this ADR must not be cited as evidence that production Runtime Reader access is live.
