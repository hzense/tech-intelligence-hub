...
Total health duration at or above five seconds fails closed. SQLSTATE `53300` and `57014` are safely classified as connection-capacity and query-cancellation failures; other query exceptions remain generic. Structured server logs may include only event, outcome, duration in milliseconds, request ID, a safe error code, SQLSTATE and pool total / idle / waiting counts. They must not include the URL, host, database, user, SQL, parameters or raw error. Pool counts remain log-only and do not independently change the public health result.

The hourly workflow reconciles a marker-bound singleton GitHub Issue after every active scheduled or manual probe. A failed probe creates, reopens or updates that Issue; a later successful probe adds one recovery comment and closes it. Only the incident job receives `issues: write`. A manual `test_alert` input may deliberately fail after the real production probe succeeds so the notification path can be tested without falsifying database failure.
...
The repository boundary is merged and CI-verified through PR #32–#35, PR #36 adds the fail-closed scheduled-health guard plus trigger/cron drift validation, and PR #38 pins the provider catalog contract observed during production preflight. A fresh seven-day Neon branch backup, role/database-ACL inventory, Runtime read-only session default and `neondb` ambient-ACL isolation are complete. The maintenance-only `hzense_migrator` connection limit was raised from 5 to 10 after Neon Tables exhausted the old ceiling and caused `53300`; that operational change does not alter Runtime privileges, the Runtime limit of 20 or the Web pool maximum of 1. After live acceptance passed, the health repository variable was set to `true`; the first controlled manual run succeeded and the hourly schedule is now enabled. PR #40 later merged the bounded error/duration classification and singleton incident reconciliation as `main@0012871`. Its exact Production deployment, direct health response, expected controlled-failure incident creation and normal recovery close passed the [2026-09-04 acceptance](../production-evidence/2026-09-04-operations-checkpoint.md#production-database-health-alert-acceptance); this does not claim direct Neon PgBouncer client-capacity or provider threshold coverage.

The target `hzense` Runtime ACL was independently checked on 2026-09-01 with two catalog-only queries: effective privileges, direct grant sources, the five-column allowlist and the bounded denylist matched the reviewed contract. On 2026-09-03, an independent Runtime credential was generated and saved through the protected workflow; the candidate provider-object contract then passed the complete target/`postgres`/`template1` production preflight twice. The [ACL evidence](../production-evidence/2026-09-01-runtime-reader-acl.md) and [preflight evidence](../production-evidence/2026-09-03-runtime-reader-preflight.md) contain no connection secret. A later, separate [production acceptance record](../production-evidence/2026-09-03-runtime-reader-production-acceptance.md) verifies the Production-only Vercel configuration, `READY` redeployment, live health response, real five-column query, safe logs and first manual run of the hourly-health workflow. This ADR defines the boundary; the dated acceptance record, not the ADR alone, is the evidence that production Runtime Reader access was live at that point. Post-acceptance credential rotation remained an open security task in that record and would have to repeat the same gates.

The [2026-09-04 operations checkpoint](../production-evidence/2026-09-04-operations-checkpoint.md) records a later bounded operator decision. The existing credential-handling exposure risk had already created the normal rotation trigger; the operator knowingly chose to defer that rotation for this work cycle without reading or changing the credential/configuration. This does not amend the least-privilege decision, lower the credential's high-sensitivity classification, claim that rotation occurred or close the rotation obligation. New evidence of misuse or additional exposure, anomalous authentication, an access/ownership change or another incident/policy trigger requires escalation; an eventual protected rotation must repeat the production gates.

The same checkpoint also makes the historical ACL recovery boundary explicit. The tracked evidence proves the current allowlist but contains no pre-normalization catalog dump, backup ID, mutation actor or mutation time, and provider backup/PITR history has not been inspected. No further destructive Runtime ACL normalization is permitted until provider history is checked and a new recoverable backup, deterministic current-state baseline and reviewed recovery rehearsal are complete. PR #42 merged a forward-only repository contract that requires a protected raw backup ID, emits and fingerprints only its domain-separated SHA-256 reference, fixes `search_path` to `pg_catalog, pg_temp`, preserves explicit-empty ACL state, and requires non-placeholder backup-reference and reviewed-fingerprint session GUCs before the first mutation. Those are declaration/integrity guards, not provider proof: output remains `providerApiVerified: false`. After an earlier CI run exposed SQLSTATE `22023` in empty ACL-array handling, the corrected head passed the complete CI run and squash-merged as `main@0806e349`; its exact Production deployment and live health contract also passed. That acceptance did not capture a production Runtime ACL baseline, inspect or verify a provider backup/PITR record, or execute a production database mutation. The merged tooling can protect a future reviewed maintenance window but cannot reconstruct or close the missing historical pre-change state. See the [bounded acceptance record](../production-evidence/2026-09-04-operations-checkpoint.md#pr-42-repository-and-deployment-acceptance).
