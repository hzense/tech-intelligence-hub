...
4. ⏳ 未执行：轮换当前 Production Runtime 凭据，并重复完整 preflight、重部署、健康/读取/日志与工作流验收。
5. ⏳ 未执行：建立连接数、池等待、查询延迟、超时和错误告警，并执行一次上线后的告警基线验证。
6. ⏳ 未执行：在稳定数据路径上继续 PostgreSQL FTS、Hybrid Search 与 Ask HZense / RAG。

截至 2026-09-03，步骤 1–3 已完成并有独立生产证据。步骤 4–6 仍待执行；小时级有界健康探针和短时间窗日志验证不能替代凭据轮换或完整的指标告警体系，也不能用本地测试、基础治理或历史 Migration / Topic 验收替代后续外部动作。
