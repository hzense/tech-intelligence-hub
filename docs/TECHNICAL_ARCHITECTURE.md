...
**状态：** 目标设计分批实施中；数据快照、任职、事件身份和事务封存已合并，正在交付私有发表转换／Outbox 底座，生产权威与权限未切换
...
`0008` 已随 PR #72 合并。本轮 `0009` [任务与发布控制](SIGNAL_PUBLICATION_CONTROLS.md)追加私有全局开关、任务策略、授权和运行四表；原始意图不可变，租约过期重领递增 fencing，终态不可恢复。私有门禁在显式短事务中按全局→任务→授权→运行锁定并使用等待后的数据库时钟；同任务领取串行化。它尚未与当前人物／证据资格、新版本组装和 Outbox 写入构成单一发表事务，未接入后台认证、AI、公开 Reader 或生产权限。
