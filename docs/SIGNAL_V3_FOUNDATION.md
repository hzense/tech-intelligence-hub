...
> **后续实施更新（2026-09-13）：** 本页保留 `0004` 首批边界。`0007` 的[事务封存与受限写入身份](SIGNAL_VERSION_IMMUTABILITY.md)已补充数据库普通 DML 不可变保护；新增 `created_xid` 是数据库元数据，不进入 `3.0.0` 指纹。发表／撤回事务、Outbox 与读取切换仍待完成，未执行生产迁移。
