...
后续增量见[人物—组织任职契约](PERSON_ORGANIZATION_AFFILIATIONS.md)与[稳定事件身份契约](SIGNAL_EVENT_IDENTITY.md)：复用旧关系方向和时间字段，追加任职证据与事件登记，不改变本页的 `3.0.0` 快照指纹。仍需补齐候选／文档模型、不可变版本保护、当前资格、发布与撤回事务、分离角色、Outbox、历史导入事务和统一读取接口。发表服务必须在事务内重新检查人物与证据、授权、当前开关、取消及租约；禁止将本批结构校验或事件键登记当成真实事实核验或可公开许可。
