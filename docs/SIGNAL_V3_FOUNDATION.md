...
> **后续实施更新（2026-09-13）：** 本页保留 `0004` 首批边界。`0007` 的[事务封存与受限写入身份](SIGNAL_VERSION_IMMUTABILITY.md)已合并；新增 `created_xid` 是数据库元数据，不进入 `3.0.0` 指纹。`0008` 私有[发表转换与 Outbox 事务底座](SIGNAL_PUBLICATION_OUTBOX.md)及 `0009` [任务与发布控制](SIGNAL_PUBLICATION_CONTROLS.md)已合并；本地交付 `0010` [已记录资格检查与原子发表](SIGNAL_QUALIFIED_PUBLICATION.md)，克隆已封存的记录合格候选并共同提交回执。完整资格化 Publisher、受控核验产物、真实消费者与读取切换仍待完成，未执行生产迁移。
...
后续增量见[人物—组织任职契约](PERSON_ORGANIZATION_AFFILIATIONS.md)、[稳定事件身份契约](SIGNAL_EVENT_IDENTITY.md)和[事务封存契约](SIGNAL_VERSION_IMMUTABILITY.md)：复用旧关系方向和时间字段，追加任职证据与事件登记，提交后保护版本和关系，不改变本页的 `3.0.0` 快照指纹。仍需补齐候选／文档模型、当前资格、发布与撤回事务、发布角色、Outbox、历史导入事务和统一读取接口。发表服务必须在事务内重新检查人物与证据、授权、当前开关、取消及租约；禁止将本批结构校验或事件键登记当成真实事实核验或可公开许可。
