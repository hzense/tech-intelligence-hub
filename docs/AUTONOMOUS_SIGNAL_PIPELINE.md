...
2026-09-13 实施增量：[V2-1a 契约](SIGNAL_V3_FOUNDATION.md) 已冻结独立 Signal `3.0.0` 快照，并实现私有版本／证据／类型化档案及历史导入预演；[任职增量](PERSON_ORGANIZATION_AFFILIATIONS.md) 增加内部任职／证据关系及保守按日判断，[事件身份增量](SIGNAL_EVENT_IDENTITY.md) 增加规范键登记与批量冲突预演。下表仍是完整目标；本批未实现语义去重服务、发表事务、Outbox、实际历史导入或生产权威切换，未知任职端点不自动构成当前雇主资格。
