...
Outbox 永久仅追加，不允许改删／TRUNCATE；两个延迟约束触发器保证事务提交时 head 精确匹配该 Signal 的最新事件，防孤儿／错误配对／状态倒退。发表／撤回使用独立发布修订号，撤回同一正文版本也产生新事件。状态转换、请求幂等和纯投影规划由内部模块实现；`0008` 本身不检查任务／策略／租约及当前人物证据资格，后续 `0010` 在私有适配器补齐记录检查，不将账本状态作为授权或合格事实。公开 Reader 与真实消费者仍未接入。详见[实施边界](SIGNAL_PUBLICATION_OUTBOX.md)。
...
`0010` 新增一张私有已记录资格发表绑定表，不直接产生公开读取许可：

| 表                                      | 主键与约束                                                                                                                                                                                                                                     |
| --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `signal_qualified_publication_receipts` | `request_key` PK；64 位十六进制请求指纹；Signal／来源版本／目标版本／run／lease owner／正 fencing token 均非空；`target_version > source_version > 0`；复合 FK 精确绑定 Outbox 的请求键、Signal 和目标版本；另有来源版本及 run 的 NO ACTION FK |
...
`0012` 新增两张私有表和一个只读安全视图：

| 对象                                   | 主键与职责                                                                                                                                 |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `signal_verification_dependency_seals` | `verification_id` PK/FK；数据库生成的精确 `dependency_seal jsonb` 和只可永久置真的 `invalidated`，历史核验不补填                           |
...
本节保留既有 Topic／FTS Reader 的生产边界。`0012` 新增的 V3 当前公开视图及最小权限增量见 [受限公开发布](SIGNAL_PUBLIC_PUBLICATION.md)；它尚未在生产授权或切换。预检结构目录精确识别新增对象，但旧 ACL 契约仍拒绝授予 Runtime 新视图读取／资格函数执行权限；生产切换须另行审核权限兼容性。
