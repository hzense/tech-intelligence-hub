...
- 30 张持久表：29 张领域、派生或私有控制表，以及 1 张 Migration 历史表；其中 8 张来自 `0004`，2 张来自 `0005`，1 张来自 `0006`，2 张来自 `0008`，4 张来自 `0009`，本轮未执行生产迁移。
- 9 个 PostgreSQL Enum。
- `vector` 扩展，以及 `search_documents.embedding vector(1536)`。
- 仓库 Migration manifest 登记十个顺序文件：`0000_foundation.sql`、`0001_radar_evidence.sql`、`0002_topic_projection.sql`、`0003_search_documents_fts.sql`、`0004_signal_version_foundation.sql`、`0005_person_organization_affiliations.sql`、`0006_signal_event_identity.sql`、`0007_signal_version_immutability.sql`、`0008_signal_publication_outbox.sql` 和 `0009_signal_publication_controls.sql`。`0003` 的历史生产执行见 [FTS-1 切换记录](production-evidence/acl/34535908960-1/cutover.md)；本批未执行 `0004`–`0009`，不声明生产已有 30 表。
- `0007` 新增 3 个 INVOKER 触发函数及 14 个 ALWAYS 触发器；`0008` 追加 2 个函数及 5 个触发器，其中 2 个是延迟约束触发器；`0009` 追加 1 个函数及 2 个 ALWAYS 触发器。合计 6 函数／21 触发器，无生产授权变化；函数正文、目录属性及 ACL 由独立精确契约核验。

物理结构的权威顺序如下：

1. [`db/migrations/*.sql`](../db/migrations/) 是 29 张应用 Schema 表的可执行 DDL 权威来源。
2. [`packages/database/src/migrate.mjs`](../packages/database/src/migrate.mjs) 创建并维护运维表 `hzense_schema_migrations`。
3. [`packages/database/src/schema.ts`](../packages/database/src/schema.ts) 是 29 张应用 Schema 表的 Drizzle 类型映射；运维历史表不进入应用 ORM 映射。
4. [`packages/database/src/verify.mjs`](../packages/database/src/verify.mjs)、[V2-1a catalog 契约](../packages/database/src/signal-foundation-catalog.mjs)、[任职 catalog 契约](../packages/database/src/affiliation-catalog.mjs)、[事件身份 catalog 契约](../packages/database/src/event-identity-catalog.mjs)、[Outbox catalog 契约](../packages/database/src/signal-publication-catalog.mjs)及[任务控制 catalog 契约](../packages/database/src/signal-publication-control-catalog.mjs)独立校验完整 30 表的列、类型、主外键、检查约束、默认值、索引、Enum、pgvector 和 Migration 历史。
...
| 表                                 | 主键与约束                                                                                                                                                                                                                    |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `person_organization_affiliations` | PK relation_id；复合 FK (relation_id,person_id,organization_id,relation_type) → relations(id,source_id,target_id,relation_type)；人物／组织档案 FK；类型限 works_at/leads/advises，职务及日期依据非空白，核验状态默认 pending |
...
`0009` 新增四张私有任务控制表，未与上述 Outbox 组成完整 Publisher：

| 表                                  | 主键与约束                                                                                                                                                      |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `signal_publication_control`        | boolean 单例 PK 且只能 true；`publication_enabled` 默认 false，迁移插入关闭记录                                                                                 |
| `signal_publication_tasks`          | UUID `task_id` PK；三种当前策略，默认 `preview_only`；任务发表开关默认关闭                                                                                      |
| `signal_publication_authorizations` | PK `(task_id,principal_id)`；task NO ACTION FK；`can_publish` 默认 false；主体 UUID 尚待认证服务绑定                                                            |
| `signal_publication_runs`           | UUID `run_id` PK；任务／主体复合 NO ACTION FK；原始意图与身份不可变；pending/running/cancelled/completed 状态与 int32 token、租约形状一致；时间有限且为毫秒精度 |

运行守卫拒绝删除、TRUNCATE、终态恢复、改写原始意图、活跃租约抢占和过期续租；重新领取递增 fencing token，最高 int32 后停止。`(task_id,status,lease_expires_at)` 索引配合任务行串行领取检查，限制同任务有效租约为一个。全局／任务／授权和运行按固定锁序读取，再使用数据库当前时钟检查控制状态；这里不验证人物证据，也不提供身份认证或安全撤回服务。详见[任务与发布控制](SIGNAL_PUBLICATION_CONTROLS.md)。
...
当前仓库目标没有 RLS 或 Policy；已由 `0007`–`0009` 明确引入并精确校验 21 个用户触发器。新增机制必须通过单独评审的新 Migration，并同步更新 Verifier 和本节；仅当变更可由 Drizzle 表达且影响应用类型映射时，才同步更新 Drizzle Schema。新增控制状态的 CHECK 校验保留括号、类型转换和字面量，防止 AND／OR 重分组被错误归一化为原约束。
...
Neon PostgreSQL 18 的保留数据库还包含 provider-owned 系统与监控对象。Production 仅在同一条 catalog 查询分别精确匹配仓库固定的两份 provider-object 合约时接受：`postgres` 为 409 项 `access`，以及 3 extensions、3 non-system Schemas、11 Relations、88 Columns、4 Indexes、1 Sequence、32 Routines、22 Types、1 Collation、1 extension Language、3 条 operator-class 支撑路径、290 项 system-object 状态、3 项 system-Schema 状态和 2 项 cluster ACL 状态；`template1` 为 298 项 `access`，以及 1 extension、1 non-system Schema、3 Routines、1 extension Language、289 项 system-object 状态、3 项 system-Schema 状态和 2 项 cluster ACL 状态，其余对象类别为 0。两库的 residual Runtime ownership、access-method、event-trigger、inheritance、operator、cast、conversion 与 text-search paths 均为 0；cluster ACL 固定两个内置 tablespaces，并覆盖 PG18 parameter ACL 的 `SET`/`ALTER SYSTEM` 能力。每类同时核对数量与 `C` 排序后的 SHA-256，覆盖 normal-OID/extension-backed system 对象、非系统对象、结构与定义、安全属性、依赖、Runtime 有效权限、grant option、direct ACL，并固定现有 system-object 显式 ACL、全 catalog Runtime ownership 与 system-Schema 完整 ACL/能力；临时 Schema 对象不进入快照。任何已纳入字段漂移、同数量替换、新增普通 OID 对象或已枚举路径均 fail closed。两库合约不可互换，也不按 `cloud_admin` owner 或名称前缀泛化。低 OID PostgreSQL 内置对象定义与 provider 原生实现仍是明确的 database-global residual，但其 ACL/ownership 权限漂移已进入门禁。
...
截至 2026-09-03，代码与 Neon 基础治理已准备：创建了新的七天 provider 回滚分支，设置 `hzense_runtime` 的 read-only session 默认值，撤销未使用 `neondb` 的 ambient database ACL，并在 Neon Tables 用满旧五连接上限后将维护专用 `hzense_migrator` 从 connection limit 5 调整到 10。Migrator 调整不改变 Runtime 权限、其 limit 20 或 Web pool 上限 1。仓库实现已通过 PR #32–#35 合并，PR #36 合并了 fail-closed 健康门禁，PR #38 固定了现场验收的 Neon provider catalog 合约；目标 `hzense` ACL 的有效权限、直接授权来源和五列 allowlist 已由两组 catalog-only 查询独立复核并保留[脱敏证据](./production-evidence/2026-09-01-runtime-reader-acl.md)。独立 Runtime 凭据已在受保护流程中生成并保存，候选 provider-object 合约又连续两次通过目标/保留库完整生产 preflight，并保存了[脱敏结果](./production-evidence/2026-09-03-runtime-reader-preflight.md)。随后五个 server-only 值仅配置到 Vercel Production；Runtime-configured 部署、健康/真实五列读取、安全日志和小时级工作流首次手工运行均通过[功能/配置生产验收](./production-evidence/2026-09-03-runtime-reader-production-acceptance.md)。2026-09-04，PR #40 合并后的精确 Production commit、直接健康合约、受控单例 incident 创建和无重复 recovery 关闭又通过[告警验收](./production-evidence/2026-09-04-operations-checkpoint.md#production-database-health-alert-acceptance)；其范围不包含 Neon PgBouncer client-capacity 或独立 provider 阈值。既有 handling-exposure risk 已构成通常的轮换触发，但操作者知情选择本轮延期，本轮未读取或修改凭据/部署配置；该决定不降低其高敏感级别，也不关闭轮换义务。同日运维检查点还确认历史 ACL 恢复材料不足，provider backup/PITR 未核验，缺口关闭前禁止再次 normalization。PR #42 在修复早期 CI 的空 ACL 数组 SQLSTATE `22023` 后完整 CI 全绿，合并为 `main@0806e349` 并通过精确 Production 部署/health 兼容性验收；该验收没有生产 ACL baseline 捕获、provider backup 核验或数据库 mutation，因此历史缺口未关闭。PR #41 随后通过最终评审、完整 CI、Search `23/23`、Web 定向 `3/3` 与精确 commit 的 Production compatibility 验收，并于 `2026-09-04T09:23:50Z` 合并为 `main@83654c48`；线上 `/search?q=OpenAI` 返回预期五条结果，数据库 health 与所选路由错误窗口通过。该[验收](./production-evidence/2026-09-04-operations-checkpoint.md#fts-0-pr-41-production-acceptance)只完成 FTS-0 canonical projection、稳定 fingerprint、进程内排序与完全平局 total-order，不包含 PostgreSQL Migration、Search Document 持久化、tokenizer/index、回填、查询 parity 或生产 cutover；FTS-1 仍未执行。上述安全/运维状态不改变本节的数据权威或权限模型。
