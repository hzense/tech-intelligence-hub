...
调用复用现有 HTTPS 域名白名单、DNS 公网检查、固定地址和证书验证，禁止重定向；SDK 不重试、不调用工具，遵守当前连接的 3–20 秒截止和 1 MiB 响应上限。20 秒内未完成的复杂请求可能成为 unknown，不能自动增加截止或重试。
...
2026-09-17 本地验证：全仓默认 2681 项测试通过；另行开启的完整原生 PostgreSQL 迁移回归 581 项通过；构建后认证／HTTP 21 项通过；候选生成真实组件浏览器测试 10 项通过。构建、类型、lint、工作流校验通过。默认跳过项不计为通过，供应商请求均为合成 transport，没有实际模型费用。任务固定链接为 `/admin/signal-generation/[id]`，查看不重新生成。
