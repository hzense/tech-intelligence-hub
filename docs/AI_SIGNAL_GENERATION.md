...
“删除任务”通过独立的认证、同源接口设置 `deleted_at`，关闭 AI 生成时仍可使用。删除从列表和任务详情移除记录，并清理本页正在跟踪的请求；保留原始状态、结果和预算去重记录，不触发模型调用。未执行任务删除时取消；执行中、结果未知以及仍有有效租约的已取消任务需等待或对账。删除已执行任务不会放开重复调用限制。
