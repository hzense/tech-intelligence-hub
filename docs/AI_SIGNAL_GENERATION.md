...
- 每条候选标题最多 50 字、摘要最多 800 字，按 Unicode 码点计数（汉字、标点、字母和空白均占额度），不是字节或 token。模型系统提示、JSON Schema 与服务端校验共用限制；超限拒绝，不静默截断。此限制用于新生成结果，不改写已有候选或正式信号；整批输出仍受 96000 字节总上限约束。
